...
d <- subset(parcel.dat, !duplicated(ProductName))
...
DeleteParcel <- function(parcelName, parcel) {
  # parcelName: product to be deleted
  # parcel: current parcel data frame
  #
  # return parce data frame after deletion
  return(subset(parcel, ProductName!=parcelName))
}
...
SimulationPernis <- function(parcel, method="statistical", n.sample=500, j.model = data.frame(jetty=c("J4","J17","J18", "J35", "J35A"), ant_from=rep(0,5), ant_to=rep(0,5))) {
    # estimate terminal time based on historical data
	#
	# parcel: parcel data frame
	# method: statistical or anticipated
	# n.sample: number of samples in simulation
	# j.model:
	# ant_from:
	# ant_to:
	#
	# return estimate terminal time based on n.sample simulation
	...
	T.terminal <- NULL
	for (i in 1:n.sample){
	  # T_processing: calculate pumping time for each sample(parcel)
	  parcel.i <- parcel %>% 
